...
def main_info_about_region():
    ...
    for index, item in enumerate(regions, 1):
        ...
        regions[item]['help'] = region_object.hospital_set.all().filter(need_help=True).count()
    return regions


def get_regions_by_name_and_id(region, page, obj_on_page=10):
    response = {}
    dict_for_transformation_name = {'br': 'Брестская область', 'vi': 'Витебская область',
                                    'go': 'Гомельская область', 'gr': 'Гродненская область',
                                    'mi': 'Минская область', 'mo': 'Могилевская область'}
    try:
        hospitals = Hospital.objects.filter(location__name=dict_for_transformation_name[region]).order_by(
            'need_help').order_by('need_help').reverse()
    except KeyError:
        response['error'] = True
    else:
